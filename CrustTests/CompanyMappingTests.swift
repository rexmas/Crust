...
class CompanyMappingTests: XCTestCase {
    ...
    func testJsonToCompany() {
        ...
        let mapper = Mapper()
        let object = try! mapper.map(from: json, using: CompanyMapping(adapter: MockAdapter<Company>()), keyedBy: AllKeys())
    ...
    func testNestedJsonToCompany() {
        ...
        let object = try! mapper.map(from: json, using: binding, keyedBy: AllKeys())
    ...
    func testUsesExistingObject() {
        ...
        let json = try! JSONValue(object: stub.generateJsonObject())
        let mapper = Mapper()
        let object = try! mapper.map(from: json, using: CompanyMapping(adapter: adapter), keyedBy: AllKeys())
